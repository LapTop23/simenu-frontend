...
export default function ModifierSheet({ item, onConfirm, onClose }) {
  ...
  const [selectedSize, setSelectedSize] = useState(hasSizes ? item.sizes[0].name : null);
  ...
  // Base price comes from the selected size's own price when this item has
  // sizes at all — never from item.price in that case, matching exactly how
  // the backend re-prices this same order line server-side (see
  // order.service.js), so this preview never disagrees with what's actually charged.
  const basePrice = hasSizes ? item.sizes.find((s) => s.name === selectedSize)?.price ?? 0 : item.price;
  const unitPrice = basePrice + flatModifiers.reduce((sum, m) => sum + m.priceDelta, 0);
  const canConfirm = !missingRequiredGroup && (!hasSizes || Boolean(selectedSize));
      ...
      <div className="animate-slide-up flex max-h-[85vh] w-full max-w-lg flex-col rounded-t-3xl bg-white">
          ...
          {hasSizes && (
            <div className="mt-3">
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-[11px] uppercase tracking-wide text-ink/40">Required · Pick 1</span>
              </div>
                ...
                {item.sizes.map((size) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-mono text-xs text-ink/60">{size.price.toFixed(2)}</span>
                    </button>
                  );
                })}
              </div>
        ...
        <div className="border-t border-sand px-5 py-4">
          ...
          <button
            type="button"
            disabled={!canConfirm}
            onClick={() => onConfirm(flatModifiers, quantity, selectedSize)}
          ...
          >
            <span className="font-semibold">
              {!selectedSize && hasSizes ? 'Select a size' : missingRequiredGroup ? `Select ${missingRequiredGroup.name}` : 'Add to cart'}
            </span>
