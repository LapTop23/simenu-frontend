 ...
 */
export async function registerOwner({ slug, restaurantName, email, password, plan, agreedToTerms }) {
  return requestJson(
    '/api/auth/register',
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ slug, restaurantName, email, password, plan, agreedToTerms }),
    },
...
export async function uploadMenuImage(restaurantSlug, file) {
  ...
  // Cloudinary's storage engine already returns a complete, permanent
  // https:// URL — only a LEGACY local-disk path (root-relative, e.g.
  // "/uploads/savory-foods/169....jpg") would ever need API_BASE_URL
  // prepended to it. Checking first avoids mangling a real Cloudinary URL
  // into something invalid like "https://api.example.com/https://res.cloudinary.com/...".
  const finalUrl = payload.data.url.startsWith('http') ? payload.data.url : `${API_BASE_URL}${payload.data.url}`;
  return { ...payload.data, url: finalUrl };
}
...
/**
 * Owner-only sales summary for the trailing N days — revenue, order count,
 * average order value, a daily revenue trend, and top-selling items.
 * Mirrors: GET /api/analytics/summary?res=<slug>&days=<n>
 */
export async function fetchSalesAnalytics(restaurantSlug, days = 7) {
  return requestJson(
    `/api/analytics/summary?res=${encodeURIComponent(restaurantSlug)}&days=${days}`,
    {},
...
/**
 * Owner-only: updates logo, theme colors, tagline, and (Premium plan only)
 * the header cover image. The backend independently re-checks the plan for
 * coverImageUrl — this isn't just a UI-level restriction.
 * Mirrors: PUT /api/restaurants/branding?res=<slug>
 */
export async function updateBranding(restaurantSlug, brandingPayload) {
  return requestJson(
    `/api/restaurants/branding?res=${encodeURIComponent(restaurantSlug)}`,
    { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(brandingPayload) },
    'Could not update your branding.'
  );
}

/**
 * Changes the logged-in owner's password. Requires the current password —
 * see auth.controller.js#changePassword for why that's non-negotiable even
 * for an already-logged-in session.
 * Mirrors: POST /api/auth/change-password
 */
export async function changePassword({ currentPassword, newPassword }) {
  return requestJson(
    '/api/auth/change-password',
    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ currentPassword, newPassword }) },
    'Could not reach the SiMenu server to change your password.'
  );
}

/**
 * Asks the restaurant's AI menu assistant a question (Premium plan only —
 * the backend independently enforces this, not just the frontend hiding
 * the widget). `history` is a short list of prior { role, content } turns
 * for conversational context.
 * Mirrors: POST /api/chatbot/ask?res=<slug>
 */
export async function askMenuChatbot(restaurantSlug, question, history = []) {
  return requestJson(
    `/api/chatbot/ask?res=${encodeURIComponent(restaurantSlug)}`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question, history }) },
    'Could not reach the menu assistant right now.'
  );
}

/**
 * Submits internal SiMenu product feedback (star rating + optional
 * comment) after an order is placed. This is NOT a public restaurant
 * review and is never shown to the restaurant owner — see
 * components/ReviewWidget.jsx and the backend's review.controller.js.
 * Mirrors: POST /api/reviews?res=<slug>
 */
export async function submitReview(restaurantSlug, { rating, comment, orderId, tableNumber }) {
  return requestJson(
    `/api/reviews?res=${encodeURIComponent(restaurantSlug)}`,
    { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rating, comment, orderId, tableNumber }) },
    'Could not submit your feedback right now.'
  );
}

// ---------------------------------------------------------------------------
// Super Admin — completely separate session from the restaurant owner auth
// system above. Different cookie, different backend routes entirely.
// ---------------------------------------------------------------------------
...
export async function logoutSuperAdmin() {
  return requestJson('/api/super-admin/logout', { method: 'POST' }, 'Could not reach the SiMenu server to log out.');
}

export async function fetchSuperAdminSession() {
  return requestJson('/api/super-admin/me', {}, 'Could not reach the SiMenu server.');
}

export async function fetchSuperAdminStats() {
  return requestJson('/api/super-admin/stats', {}, 'Could not load restaurant stats.');
}

export async function fetchSuperAdminReviews(page = 1, limit = 50) {
  return requestJson(`/api/super-admin/reviews?page=${page}&limit=${limit}`, {}, 'Could not load reviews.');
}
